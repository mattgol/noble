...
const Hci = function () {
  ...
  this._state = null;
  this._deviceId = null;
  this._connectionstate = 'leconn_idle';
...
Hci.prototype.createLeConn = function (address, addressType) {
  ...
  cmd.writeUInt16LE(0x0050, 19); // max interval
  cmd.writeUInt16LE(0x0002, 21); // latency
  cmd.writeUInt16LE(0x0200, 23); // supervision timeout
  cmd.writeUInt16LE(0x0004, 25); // min ce length
  cmd.writeUInt16LE(0x0006, 27); // max ce length

  debug(`create le conn - writing: ${cmd.toString('hex')}`);
  this._connectionstate = 'leconn_requested';
  this._socket.write(cmd);
};
...
Hci.prototype.disconnect = function (handle, reason) {
  if(!handle) {
    if(this._connectionstate === 'leconn_pending') {
      this.cancelConnect();
    } else if(this._connectionstate === 'leconn_requested') {
      this._connectionstate = 'leconn_canceling';
    }
    return;
  }
...
Hci.prototype.processCmdStatusEvent = function (cmd, status) {
  if (cmd === LE_CREATE_CONN_CMD) {
    if(this._connectionstate === 'leconn_canceling') {
      this.cancelConnect();
      return;
    }
    this._connectionstate = 'leconn_pending';
    if (status !== 0) {
